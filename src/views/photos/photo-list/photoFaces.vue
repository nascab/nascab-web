<template>
	<div class="album-root-wrapper">
				...
				<vs-card v-for="(item, index) in dataList" type='1' @click="onAlbumClick(index)"
					...
					<template #title>
						<div
							...
							<p class="item-name max-line-one">{{ item.face_name ? item.face_name : $t('photo.unnamed')
							}}</p>
						</div>
					</template>
					<template #img>
						<img style="object-fit: cover;pointer-events: none;" v-if="item.url"
							:style="{ 'width': itemWidth + 'px', 'height': itemWidth + 'px' }" :src="item.url" />
		...
		<vs-dialog v-model="showLongPressMenu">
			<h4 v-if="selectedItem" class="max-line-one" style="word-break:break-all">{{ selectedItem.face_name ? selectedItem.face_name : $t('photo.unnamed') }}</h4>
...
export default {
	...
	methods: {
		enableAiFace() {
			this.api.post('/api/commonApi/saveConfig', { aiFaceEnable: '1' }).then((res) => {
				this.getFacesList()
			}).catch((error) => { })
		},
		// 退出选择模式
			...
			if (this.selectMode) {
				...
				this.$router.push({
					...
					params: {
						...
						faceId: this.selectedItem.id + '',
						showSearchBtn: false,
						title: this.selectedItem.face_name ? this.selectedItem.face_name : this.$t('photo.unnamed')
					}
				});
			} else {
