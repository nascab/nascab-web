<template>
	<div class="photo-list-root" ref="photoListRoot">
		...
		<div class="photo-header">
			<photo-operation-header ref="photoHeader" @showSearchBtn="showSearchBtn"
				@onChooseDataType="onChooseDataType" @onChangeShowMode="onChangeShowMode" @onSizeChange="setImgSize"
				:editMode="editMode" @onChooseRange="onChooseRange" :nearMode="nearMode" @onSearch="onSearch"
				@albumId="albumId" @onSwitchEditMode="switchEditMode">
...
export default {
	...
	activated() {
		console.log('photoList activated ')
		if (this.timeLine) {
			//加载第一页照片
			this.loadFirstPage();
		}
		...
		onPhotoClick(index) {
			...
			if (this.editMode) {
				...
				if (photoList[index].type == 1) {
					...
					this.$nextTick(() => {
						this.$refs.videoPlayer.playVideo(photoList, index, this.$refs.photoHeader.allowAndroidPlayRaw);
					});
				}
			}
		},
